...
if (!listRes.ok) {
  ...
  setMsg(`Erreur API /clients: ${listRes.status} ${text}`);
  setRows([]);
  setLoading(false);
  return;
}
      ...
      <Card className="mt-4 rounded-2xl">
        <CardHeader className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex w-full gap-2 md:w-[360px]">
            <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Rechercher (nom, ID…)" />
        ...
        <CardContent>
          {msg ? (
            <div className="mb-3 rounded-xl bg-muted p-3 text-sm">{msg}</div>
          ) : null}
