...
export default function OrganizationsPage() {
  ...
  // Toujours tenter un refresh pour être sûr
  ...
  // si refresh échoue, on essaie avec token existant
  return token ?? null;
}
  ...
  async function loadAll() {
    ...
    if (!res.ok) {
      ...
      setMsg(`Erreur /organizations: ${res.status} ${text}`);
      setOrgs([]);
      setLoading(false);
      return;
    }
  ...
  async function createTenantAdmin() {
    ...
    const res = await fetch(`${API_BASE}/organizations/create-admin`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
      ...
      {role && role !== "SUPER_ADMIN" ? (
        <Card className="mt-4 rounded-2xl">
          ...
          <CardContent className="text-sm text-muted-foreground">
            Cette page est réservée au SUPER_ADMIN.
          </CardContent>
        </Card>
      ) : null}
